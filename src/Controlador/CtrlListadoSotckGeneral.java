 ...
 * @author JFAA
 */
public class CtrlListadoSotckGeneral implements ActionListener{
    private Producto pro;
    private ConsultaProducto conPro;
    private jfListadoStockGeneral jfPro;
    private ConsultaCategoria conCat=new ConsultaCategoria();
    private ConsultaMarca conM=new ConsultaMarca();
    
    public CtrlListadoSotckGeneral(Producto pro, ConsultaProducto conPro, jfListadoStockGeneral jfPro){
        this.pro=pro;
        this.conPro=conPro;
        this.jfPro=jfPro;
    ...
    public void listar(){
        //Variables listas
        long cantidadPro = 0, totalPrecio=0, totalCosto=0, precios=0,costos=0;        
        ...
        try{
            listaDetalle=conPro.obtenerProductos();
            ...
            if(!listaDetalle.isEmpty()){
                ...
                for(int i=0; i<listaDetalle.size();i++){
                    ...
                    object[3]=listaDetalle.get(i).getExistencia();
                    precios=listaDetalle.get(i).getPrecio()*listaDetalle.get(i).getExistencia(); 
                    costos =listaDetalle.get(i).getCosto()*listaDetalle.get(i).getExistencia();
                    object[4]=costos;                    
                    object[5]=precios;
                    dtmDetalle.addRow(object);
                    cantidadPro = cantidadPro +1;
                    totalPrecio=totalPrecio+precios;
                    totalCosto=totalCosto+costos;                    
                }
                //hacer visible los datos en el JTable
                jfPro.jtListado.setModel(dtmDetalle);
                jfPro.jlTotalProductos.setText(String.valueOf(cantidadPro));
                jfPro.jlTotalCosto.setText(String.valueOf(totalCosto));
                jfPro.jlPrecio.setText(String.valueOf(totalPrecio)); 
